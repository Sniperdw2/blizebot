...
bot.on('ready',(msg) => {
    ...
    })(chalk.yellow('BlizeBot | SelfBot created by ElCarisme666 '));
...
bot.on('disconnect', event => {
    if (event.code === 1000) {
        logger.info('Déconnecté par une erreur discord');
    } else if (event.code === 4004) {
        // Force the user to reconfigure if their token is invalid
        logger.severe(`Erreur dans la procedure de connexion . Le token est invalide ! ${chalk.green('yarn run config')}.`);
        process.exit(666);
    } else {
        logger.warn(`Déconnecté par discord avec le code ${event.code}`);
    }
});
